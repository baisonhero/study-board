#!/usr/bin/env python3
"""
Build V9 of Claude Code 月次利用量レポート (A4 portrait, single-page dashboard).

V9 changes vs V8 rev3 (per user feedback):
  1) Unit standardization: every cost expressed in [千円] (k/M notation dropped).
       ¥9.9M → 9,900 [千円] ; ¥822k → 822 [千円] ; ¥1.5M → 1,500 [千円] ; etc.
       効率TOP3 unit "行/¥1k" → "行/千円".
  2) Monthly trend chart split into two side-by-side panels:
       LEFT 58% : dual-axis trend chart (kept from V8 rev3).
       RIGHT 42%: NEW "自プロジェクトメンバー コストTOP5" panel — real names,
                  cost-only horizontal bars + value.
  3) Member-row TOP3 panels reduced from 3 → 2 (コスト panel removed):
       生成行TOP3 / 効率TOP3 only, both panels widened to fill the space.
  4) Version pill → V9; date → 2026-05-09.
...
add_textbox(slide, "V9",
            l=6.95, t=0.27, w=1.02, h=0.28,
            size=14, color=AMBER, bold=True, align='center', valign='middle')

add_textbox(slide, "2026年3月度  /  報告者: Ogata  /  報告日: 2026-05-09  /  対象期間: 2026/03/01 - 03/31",
...
mpl_chart_png = "/tmp/v9_monthly_trend.png"
...
add_textbox(slide,
            "効率は改善継続、年度末予測 9,900 [千円] = ミドル6.6人月相当。次手は『増員 vs Claude活用拡大』の経営判断",
...
out = "/sessions/lucid-festive-wozniak/mnt/_outputs/2026-05-09_Claude_monthly_report_V9.pptx"
prs.save(out)
print("WROTE:", out)
