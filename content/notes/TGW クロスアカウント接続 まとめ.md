...
- C の TGW RT は `propagation`（伝播）を有効にすればアタッチ時に自動で埋まる。静的運用なら C が `static route`（静的ルート）で CIDR を 2 本書く（各用語の定義と設定方法は §5）
...
## 5. association / propagation / static route — 用語と設定方法

TGW ルートテーブル周りで混同しやすい 3 つの用語。**3 つとも TGW ルートテーブル上の設定で、操作するのは TGW 所有者のアカウント C**。association と propagation は「対義語」ではなく、向きの違う別々の 2 つの紐付けである点に注意（§5.4）。

### 5.1 association（関連付け）

- **定義**: アタッチメントが「**どの TGW ルートテーブルを引くか**」の紐付け。そのアタッチメントから TGW に入ってきた通信は、association されたルートテーブルを見てルックアップする
- **数の関係**: アタッチメント 1 個 → ルートテーブル **ちょうど 1 個**（多対一）
- **設定方法（C）**: TGW ルートテーブルの「Associations」で対象アタッチメントを選んで関連付ける。TGW 作成時に *Default association route table* を有効にしておけば、新規アタッチ時に既定ルートテーブルへ自動で association される

### 5.2 propagation（伝播）

- **定義**: アタッチメントの経路（VPC CIDR）を「**どの TGW ルートテーブルに自動で載せるか**」。有効にすると、そのアタッチメントの VPC CIDR が対象ルートテーブルに自動で書き込まれる
- **数の関係**: アタッチメント 1 個 → ルートテーブル **0〜複数個**
- **設定方法（C）**: TGW ルートテーブルの「Propagations」で対象アタッチメントを選んで伝播を有効化する。TGW 作成時に *Default propagation route table* を有効にしておけば、新規アタッチ時に自動で伝播される

### 5.3 static route（静的ルート）

- **定義**: propagation を使わず、人が手で「宛先 CIDR → アタッチメント」を TGW ルートテーブルに書く方法。propagation の **代替手段**
- **設定方法（C）**: TGW ルートテーブルの「Routes」→ *Create static route* → 宛先 CIDR とターゲットアタッチメントを指定する。`0.0.0.0/0` や、特定 VPC を意図的に隔離する blackhole ルートもここで書く
- propagation（自動）と static route（手動）は併用でき、両方ある場合は **static route が優先**される

### 5.4 関係の整理

| 用語 | 何を決めるか | 向き | 数の関係 |
|---|---|---|---|
| association | このアタッチメントが**引く** RT | アタッチメント → 1 枚の RT を使う | 1 : 1 |
| propagation | このアタッチメントの CIDR が**載る** RT | アタッチメント → RT に経路を広告 | 1 : 0〜N |
| static route | RT に経路を**手で書く** | propagation の代替（手動） | — |

- **association と propagation は対義語ではない** — 向きの違う「別々の 2 本の紐付け」。両方ともアタッチメントごとに設定する（既定ルートテーブルを使う設定なら、アタッチ時に両方とも自動化できる）
- **propagation の対になるのは static route** — ルートテーブルに経路が入る手段の「自動（propagation）／手動（static route）」
- これらは**すべて TGW ルートテーブルの概念**。VPC 側のサブネットルートテーブル（§4）には propagation は効かず、そちらは A・B が手で `相手 VPC CIDR → TGW` を書く

## 6. パケットの流れ（B の Lambda → A の Aurora）
