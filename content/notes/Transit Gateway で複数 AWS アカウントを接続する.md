...
- → A の Aurora SG で許可するのは、ワークロード private サブネット 3 つの CIDR（`10.1.0.0/20` / `10.1.16.0/20` / `10.1.32.0/20`）を 3 行
- 面倒なら VPC B の CIDR `10.1.0.0/16` を 1 行で許可すれば全サブネットを包含できる（TGW 専用サブネットも含むが、そこは送信元にならないので実害なし）
