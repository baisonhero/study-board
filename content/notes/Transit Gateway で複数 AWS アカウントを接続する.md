...
## 17. TGW セグメンテーション — ルートテーブル分離でスポークを隔離

### 17.1 「TGW に繋ぐ = 全員と繋がる」ではない

TGW にぶら下がった VPC が、ルートテーブルを何も意識せず全員相互に到達できてしまうのか？ → **そうなるのは「C が単一のフラットなルートテーブルに全 VPC を propagate した場合」だけ**。それは設定上の選択であって TGW の必然ではない。

- TGW は内部に **複数のルートテーブル**を持てる
- 各アタッチメントには **association（どのルートテーブルを見るか）が 1 つ**決まる
- 「ある VPC が何に到達できるか」= 「その VPC のアタッチメントが association しているルートテーブルに、どの宛先ルートが入っているか」で決まる
- → 到達範囲は **C が TGW ルートテーブル設計で完全に制御できる**。スポークは自分の VPC RT をどういじっても、TGW ルートテーブルに無い宛先には行けない

### 17.2 ★ ルートテーブルのエントリ構造 — 「送信元」欄は無い

セグメンテーションを理解する前提として重要: **ルートテーブルのエントリに「送信元」の欄は無い**。

- エントリは常に **`宛先(destination) CIDR → ターゲット(target)`** の形。ルーティングは**宛先ベース**のみ
- 「VPC B のレンジ → Z のレンジ」のような書き方は誤り:
  - 左辺は「送信元（B のレンジ）」ではなく **宛先**。自分のレンジは書かない
  - 右辺（ターゲット）は「Z の IP レンジ」ではなく **アタッチメント**（or blackhole 等）。CIDR は書かない
- `0.0.0.0/0` は **宛先**として使える（「あらゆる宛先」）。**送信元として使う欄は存在しない**
- 「誰の（B の）ルートテーブルか」は **association で決まる**。ルートの中身に「B」という送信元が出てくるわけではない

### 17.3 B を Z（共通 NAT / 共有サービス）だけに繋ぐ設計例

B を Z（例 `10.9.0.0/16`）だけに繋ぎ、他スポークへは行かせたくない場合、C はこう組む:

1. **スポーク用 TGW ルートテーブル**を作り、**B のアタッチメントをそれに association**
2. そのルートテーブルの中身は **Z への route だけ**:

```
宛先              ターゲット
10.9.0.0/16       Z のアタッチメント        ← Z だけに繋ぐ場合
```

Z が egress でインターネット含め全部 Z 経由なら:

```
宛先              ターゲット
0.0.0.0/0         Z のアタッチメント        ← この 0.0.0.0/0 は「宛先」
```

3. **他スポーク（X, Y…）への route は 1 行も入れない**

→ B のトラフィックが TGW に来て B 用ルートテーブルを引いても、X の CIDR にマッチする行が無い → 行き先なし → **B は X に到達できない**。「送信元でブロック」ではなく「**B が見るルートテーブルに X への道を書いていない**」という消極的遮断。これが TGW セグメンテーションの実体。

**B の VPC RT が `0.0.0.0/0 → TGW` でもバイパス不可**: B が自分の VPC ルートテーブルをどう書いても、TGW に来た後は B 用ルートテーブルに従う。そこに Z への道しか無ければ B は Z にしか行けない。

**戻り（Z → B）**: Z のアタッチメントは別のルートテーブルに association し、そちらに `10.1.0.0/16（B）→ B のアタッチメント` を入れておく（戻り通信用）。

### 17.4 セグメンテーションの道具

- **複数 TGW ルートテーブル + association 設計** — どのアタッチメントがどのルートテーブルを見るかで到達範囲を仕切る（主役）
- **propagation を絞る** — スポークのアタッチメントを互いのルートテーブルに propagate しなければ、スポーク同士は自動でも繋がらない
- **blackhole ルート** — 特定 CIDR を明示的に「破棄」と書ける（「ここには絶対行かせない」を明示）

### 17.5 典型パターン

| パターン | TGW ルートテーブル設計 | 到達範囲 |
|---|---|---|
| フラット | 1 ルートテーブル・全 propagate | 全 VPC が相互到達 |
| 分離スポーク | スポーク用 RT は共有サービス / egress への route だけ | スポーク → 共有サービスのみ。スポーク同士は不可 |
| 共有サービス / egress VPC | その VPC の RT はスポークへ戻れる route を持つ | 戻り通信成立 |

→ 「TGW に繋ぐ = 全員と繋がる」ではない。**繋がる範囲は C が TGW ルートテーブルの association 設計で決める**。「B → 共通 NAT だけ許して横の通信は塞ぐ」はまさに標準的な構成。
...
## 補足3. 既接続の VPC から、TGW 上の別の宛先を追加するには

> シナリオ: 自分の VPC は既に C の TGW に繋がって稼働中。追加で TGW 上の**別の宛先（別 VPC D など）**に通信したくなった。「新しいアタッチメントを作る？ 既存アタッチメントを編集する？ それともルートテーブルだけ変えればいい？」

**大原則: アタッチメントは「自分の VPC ⇄ TGW」を繋ぐもの。1 VPC = 1 アタッチメント。「接続先ごと」に作るものではない。** 一度自分の VPC を TGW に繋げば、その 1 本のアタッチメントが TGW 経由の**全宛先**（A でも D でも）のトラフィックを運ぶ。

→ 自分の VPC が既に TGW 接続済みなら、新しい宛先のために:

- **アタッチメントの追加も編集も不要**
- やるのは **ルートテーブルへのルート追加**

ただし「ルートテーブルだけ」には 2 つの重要な注意がある。

### 注意(1) propagation は VPC サブネット RT には効かない

- propagation が自動で埋めるのは **TGW ルートテーブル（C 所有）だけ**。**自分の VPC のサブネットルートテーブルには一切作用しない**
- → 新しい宛先 D が増えたら、**自分の VPC サブネット RT に `D CIDR → TGW` を手動で追加するのは必須**（propagation 有効でも無関係。IaC 管理ならコードに追記）
- 「propagation されてるから VPC 側のルートテーブルすら変えなくていい」は**誤り**

### 注意(2) 相手（D）側のサブネット RT にも自分の CIDR が要る — 片側だけでは不成立

- 通信は往復。B が `D CIDR → TGW` を入れて往路が通っても、**D が応答を B に返すには D のサブネット RT に `B CIDR → TGW` が必要**
- D の RT に B 向けルートが無いと、D は応答パケットの行き先が分からず破棄 → TCP は成立しない（「行きは届くのに繋がらない」典型ハマり。§5.7 と同じ理屈）

### まとめ

| ルートテーブル | 追加内容 | やる人 |
|---|---|---|
| 自分（B）の VPC サブネット RT | `D CIDR → TGW` | B（手動・必須） |
| 相手（D）の VPC サブネット RT | `B CIDR → TGW` | D（手動・必須、相手に依頼） |
| C の TGW ルートテーブル | 追加不要（B・D とも既にアタッチ済み + propagation 前提） | — |

- 既接続 VPC からの宛先追加 = **アタッチメント不要**、**両 VPC のサブネット RT に相手 CIDR → TGW を手動追加**
- propagation は TGW ルートテーブル側の自動化であって、VPC サブネット RT には無関係
- そのほか D 側のリソースに繋ぐなら、D 側 SG で B の CIDR 許可も必要
- ※ 一方、「追加したい先が**自分のアカウントの別 VPC**」の場合は話が別。その新 VPC には**新しいアタッチメントが必要**（§4.1 (3) 参照）

## 補足4. NACL デフォルト時、人が書く SG ルールは A 側 1 本だけ

> Q: NACL が未設定（デフォルト）なら、SG はステートフルだから A の SG だけで通信でき、B 側は SG 設定不要？

**ほぼその通り。正確には「B は SG にルールを追加しなくてよい」。**

- **NACL**: 両側デフォルト（未設定）= 全方向 ALLOW。NACL はステートレスだが、デフォルトが全許可なので気にしなくてよい
- **SG（ステートフル）**:
  - B の Lambda にも SG は必ず付く（ENI には必ず SG が付く）。ポイントは **新規 SG のデフォルト アウトバウンドが「全許可」**であること。これを変えていなければ Lambda は Aurora の 3306/5432 へそのまま出られる → **B 側はルール追加不要**
  - SG はステートフルなので **戻りトラフィック（Aurora の応答）は自動で許可**される。B の SG にインバウンドルールも、A の Aurora SG にアウトバウンドルールも、応答のためには不要
  - → **この接続で人が書く必要がある SG ルールは「A の Aurora SG インバウンド = B のワークロード CIDR : 3306/5432」の 1 本だけ**
- **唯一の例外**: B が Lambda SG のアウトバウンドをデフォルトの全許可から絞っている場合だけ、B 側に `A CIDR : 3306/5432` のアウトバウンドルールが要る

→ NACL デフォルト前提なら「**A の Aurora SG 1 本だけ書けば通信成立**」で正しい。
