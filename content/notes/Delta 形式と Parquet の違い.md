...
## 5. Plain Parquet でクエリするのと Delta でクエリするのは何が違うか

「Athena で S3 の Parquet を SELECT する」だけなら Delta 不要でも動く。じゃあ Delta を使うと何が変わるか、**事前準備**と**クエリ実行時の動き**の 2 軸で並べる。ここが Delta の存在意義を一番具体的に感じる部分。

### 5.1 事前準備の比較

**Plain Parquet の場合** — 自分で色々見る必要がある:

- ファイルを S3 に配置（Hive パーティション設計は自分で: `s3://bucket/table/year=2026/month=07/`）
- Glue Catalog / Athena で **`CREATE EXTERNAL TABLE`**（スキーマを手で書く）
- パーティションを**登録**: `ALTER TABLE ADD PARTITION` or `MSCK REPAIR TABLE`
- 新規パーティション追加のたびに**登録処理**が必要（自動発見設定もあるが遅い）
- 書き込み中の中途半端なファイルが読まれないよう、書き込み側で工夫

**Delta の場合** — メタデータが `_delta_log/` にあるので事前登録が薄い:

- ファイルは Delta writer（Spark / Glue / EMR）が `_delta_log/` と一緒に配置
- Athena では `TBLPROPERTIES ('table_type' = 'DELTA')` で外部テーブル定義（**LOCATION だけ指定**）
- **スキーマ手書き不要**（`_delta_log/` から読む）
- **パーティション登録不要**（`_delta_log/` にファイル一覧が全部載っている）
- **`MSCK REPAIR` 不要**
- 書き込み中でも読み手には commit 済みのバージョンしか見えない（ACID）

### 5.2 クエリ実行時の動き

**Plain Parquet で `SELECT ... WHERE event_date = '2026-07-01'` を投げると**:

```
Athena
  ① S3 LIST bucket/prefix/                      ← 対象パーティション以下のファイル一覧を取得
  ② パーティションプルーニング                    ← WHERE 句のパーティションキーで絞る
  ③ 残った Parquet ファイルの footer を全部読む   ← 各ファイルの min/max stat を確認
  ④ predicate に該当するファイルの列を読み出す
  ⑤ 集約 → 結果返す
```

- ★ **① の S3 LIST が遅い**。数万〜数百万ファイルあると LIST がボトルネック（1 リクエストで最大 1000 ファイル、ページング必要）
- ★ **③ で全ファイルの footer を読む**必要がある。ファイル数が多いと開くだけで重い

**Delta で同じクエリを投げると**:

```
Athena
  ① S3 GET _delta_log/*.json                    ← 通常数十 KB、1〜数回で終わる
  ② ログから「今のスナップショットのファイル一覧」と「各ファイルの min/max stat」を取得
  ③ predicate と stat でファイルスキッピング     ← ここで大半のファイルが除外される
  ④ 対象ファイルの列だけ読み出す
  ⑤ 集約 → 結果返す
```

- ★ **① で S3 LIST が要らない**。ログにファイル一覧が確定的に載っている
- ★ **② で min/max も一緒に取れる**ので、ファイルを開かずに読み飛ばし判定できる
- 結果として**メタデータ操作のオーバーヘッドが桁で減る**

### 5.3 実際どのくらい違うか

- **少量ファイル（数百）**: 体感差なし。Plain Parquet でも十分速い
- **中規模（数千〜数万ファイル）**: Delta の方が明らかに速い（LIST が節約できる）
- **大規模（10 万ファイル超）**: Delta の**メタデータ層が生きる**。Plain Parquet だと LIST だけで秒〜十秒単位
- **同時読み書き**: Plain Parquet は稀に不整合が見える。Delta は ACID で読み手が壊れない
- **UPDATE / DELETE / MERGE**: Plain Parquet は不可（ファイル再書き込みが必要）、Delta は SQL DML でそのまま可

### 5.4 まとめ表 — 事前準備と動きの違い

| | Plain Parquet | Delta |
|---|---|---|
| CREATE TABLE 時のスキーマ定義 | 手動 | 不要（`_delta_log/` から読む） |
| パーティション登録 | `ADD PARTITION` / `MSCK REPAIR` | 不要 |
| 新規ファイル追加時の登録 | 必要 | 自動（writer が `_delta_log/` 更新） |
| クエリ時の S3 LIST | 必要 | **不要**（ログにファイル一覧） |
| min/max stat の取得 | 各ファイル footer を読む | **ログにまとまってる** |
| 書き込み途中の見え方 | 中途半端が見える可能性 | commit 前は非公開（ACID） |
| UPDATE / DELETE / MERGE | 不可（自分でファイル管理） | SQL DML で可 |
| タイムトラベル | 不可 | `VERSION AS OF n` で可 |

**ざっくり**: **少量なら Parquet 直で十分。中〜大規模、あるいは書き込みが頻繁なら Delta のメタデータ層の恩恵が大きい**。「事前準備を減らして高速クエリしたい」なら Delta（や Iceberg）が答え。

## 6. Delta vs Iceberg vs Hudi — テーブルフォーマット 3 強
...
### 7.1 Amazon Athena — マネージド SQL クエリ
