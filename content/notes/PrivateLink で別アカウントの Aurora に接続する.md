...
> [!summary]
> 前回 [[Transit Gateway で複数 AWS アカウントを接続する]] / [[TGW クロスアカウント接続 まとめ]] で扱った「B の Lambda/ECS から A の Aurora にクエリする」と同じ用件を、今度は **PrivateLink** で実装する場合の設計メモ。**Aurora は PrivateLink で直接公開できず NLB＋[[RDS Proxy]] 経由が定石**であること、CIDR 重複が許容される代わりに公開範囲が NLB:port に絞られること、ハブアカウント C が不要なこと、を中心に整理。アーキテクチャ図・責務マップ・TGW との比較表・A/B の実施事項リスト・**B からの呼び出し方（コード例）**・判断軸を含む。
...
## 6. B からの呼び出し方 — Lambda は普通の DB 接続をするだけ

PrivateLink のエンドポイントが B 側に立つと、**B の Lambda / ECS は NLB のことを知らずに、自分の VPC 内のホスト名に普通に TCP で接続するだけ**で済む。「Interface Endpoint と Endpoint Service がいい感じにルーティングしてくれる」という認識で正しい。

### 6.1 Lambda から見た景色 — コード例

```python
import pymysql, json, boto3

# 1. B 側の Secrets Manager から DB 認証情報を取得
secret = boto3.client('secretsmanager').get_secret_value(SecretId='aurora-from-A')
creds = json.loads(secret['SecretString'])

# 2. Interface Endpoint のホスト名に対して接続する
conn = pymysql.connect(
    host='vpce-0abc123-xxxxxx.vpce-svc-0def456.ap-northeast-1.vpce.amazonaws.com',
    port=3306,                 # ← A の NLB と同じポート
    user=creds['username'],
...
with conn.cursor() as c:
    c.execute('SELECT * FROM v_for_b WHERE ...')
    rows = c.fetchall()
```

ポイント:

- 接続先 = **Interface Endpoint のホスト名**（`vpce-xxx....amazonaws.com`）。これは **B が Interface Endpoint を作った時点で AWS が自動発行する DNS 名**。手で組み立てるものではなく、コンソール／API が返す値をそのまま使う
- ポート = **A の NLB と同じポート**（3306 / 5432）。PrivateLink はポート変換しない
- NLB のホスト名は B からは見えないし、見る必要もない

### 6.2 裏で何が起きているか — パケットの旅

```
Lambda (B)
  └─ DNS 解決: vpce-xxx... → B の VPC 内の IP（例 10.1.255.5）
  └─ TCP 接続: 10.1.255.5:3306
       └─ Interface Endpoint ENI（B の VPC 内）
            └─ PrivateLink（AWS backbone を経由）
                 └─ A の Endpoint Service
                      └─ A の NLB
                           └─ RDS Proxy
                                └─ Aurora
```
...
1. Lambda が `vpce-xxx....amazonaws.com` を DNS 解決 → **B の VPC 内の IP**（Interface Endpoint ENI の IP）が返る
2. Lambda はその IP に普通に TCP 接続を張る。**自分の VPC 内のローカル IP を叩いているように見える**
3. その IP は実は Interface Endpoint の ENI で、AWS がそこから PrivateLink バックボーン経由で A の Endpoint Service に届ける
4. Endpoint Service → NLB → RDS Proxy → Aurora。応答は逆経路

つまり **B 側は B の VPC 内のローカル IP に繋いでいるだけ**で、その先の A 側の構成は PrivateLink 層が透過的に扱ってくれる。

### 6.3 vpce-xxx ホスト名を綺麗にしたい場合（任意）

毎回 `vpce-0abc123-xxxxxx....amazonaws.com` を書くのは煩雑なので、現場では:

- **Private DNS Name 機能（A 側で設定）**: A が Endpoint Service に `db.acompany.internal` のような Private DNS 名を設定 → B の VPC でそのまま `db.acompany.internal:3306` で接続できる（A 側はドメイン所有検証が必要）
- **B 側 [[Route 53]] Private Hosted Zone**: B が自分の VPC に Private Hosted Zone を切って、`aurora-from-a.internal.b.com` を `vpce-xxx` の CNAME にする。Lambda は綺麗な名前で接続できる

どちらも「ホスト名を綺麗にする飾り」であって、本質（Interface Endpoint ENI に繋ぐ → PrivateLink トンネル）は変わらない。

## 7. 認証・認可は別レイヤ（PrivateLink は関与しない）
